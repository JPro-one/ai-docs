...
If `build/ai-docs/` doesn't exist or is empty, generate it by running:

```
gradle collectDocs
```

Also re-run this when the docs may be stale: if the project's build files (where dependencies are declared) are newer than `build/ai-docs/index.md`, the collected documentation no longer matches the dependency set.
