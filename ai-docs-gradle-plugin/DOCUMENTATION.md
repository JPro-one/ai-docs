...
For each dependency, it attempts to resolve a `DOCUMENTATION.md` classifier artifact and a `sources.jar`. Dependencies that publish neither are silently skipped.
...
|---|---|---|---|---|
| `DOCUMENTATION.md` | `DOCUMENTATION` | `md` | At least one of DOCUMENTATION.md or sources.jar | Full library documentation |
| `sources.jar` | `sources` | `jar` | At least one of DOCUMENTATION.md or sources.jar | Source code for AI to reference |
| POM | — | `pom` | No (auto-resolved) | Extracts name, description, homepage, license |

Dependencies that publish **neither** a `DOCUMENTATION.md` nor a `sources.jar` are silently skipped. If at least one is present, the library appears in the index.
