...
- JPro File (125 lines 1-125)
  - Introduction (82 lines 3-84) — A library for handling file related operations (like open, save and drag & drop) independently of the running platform,
      - Limitations (5 lines 80-84) — Some combination of features only work limited. These are the following:
  - Installation (20 lines 85-104) — Add the following configuration to your project based on the build tool you are using:
  - Launch the examples (21 lines 105-125) — [**Text Editor sample**](https://github.com/JPro-one/jpro-platform/blob/main/jpro-file/example/src/main/java/one/jpro...
...
- JPro Routing (186 lines 1-186)
  ...
  - Basic API (42 lines 73-114)
    - Route (19 lines 75-93) — Route static methods:
    - Response (10 lines 94-103) — Response static methods:
    - Request (11 lines 104-114) — Methods of Request:
  ...
  - Additional Features (17 lines 170-186)
    - Incremental Loading (15 lines 172-186) — It's possible to load parts of the application incrementally.
...
- Maven (256 lines 44-299) — mvn jpro:run
  - Gradle setup (99 lines 51-149) — The simplest way to begin is by using **Gradle** as your build tool.
    ...
    - `3` Run the app (10 lines 140-149) — In a terminal session, navigate to the main project directory and enter the command:
  - Maven setup (114 lines 150-263) — JPro provides a plugin for Maven, which allows you to easily start JPro from an existing project.
    - `1` Install Maven (4 lines 156-159) — Maven can be downloaded and installed [here](https://maven.apache.org/).
    - `2` Create the Maven script (94 lines 160-253) — Create the file `pom.xml` and put it into your **project’s root directory**.
    - `3` Run the app (10 lines 254-263) — Start a **Terminal session**, move to the **main project directory** and enter the command:
  - Build commands (36 lines 264-299) — The `gradle` & `maven` JPro plugins provide simple commands to **run your application locally** during development as...
    - Local Development (17 lines 272-288) — To test your application during development, the `jproRun` command will run your app on [localhost](http://localhost).
    - Production Release & Server Commands (11 lines 289-299) — When it’s time to release or run your JPro server remotely, you can use the following commands to create a **zip-file...
- Using JPro (486 lines 300-785)
  - Build configuration (100 lines 302-401) — JPro properties below are available in both Gradle & Maven.
    - JPro-only dependencies with Gradle (12 lines 327-338) — In Gradle, `jproOnly` can be used to configure any dependencies which should only be used when running the applicatio...
    - Example `build.gradle` configuration (26 lines 339-364) — For Gradle, properties are set inside of `jpro { ... }`.
  ...
  - JPro config (52 lines 402-453) — The following properties can be set in the `jpro.conf` file:
    - Declaring multiple runnable applications (23 lines 431-453) — For an app to be **JPro enabled**, it must extend the class called `javafx.application.Application`.
  - HTTP routes & resources (71 lines 454-524)
    - Home route (6 lines 456-461) — In general, the `index.html` file is the default resource within each folder. This means that [http://localhost:8080/...
    - Static resources (10 lines 462-471) — Any resources underneath `jpro/html/` are made publicly available by the JPro server. These can be accessed via URL b...
    - Adding a global default resource (8 lines 472-479) — JPro allows you to define a default resource that will be returned when a URL does not find a specific resource. This...
  ...
  - JavaFX node attributes (31 lines 525-555) — In some cases, you may wish to override certain behavior for a specific element or area of your application instead o...
    - **JavaFX Window Attributes** (8 lines 531-538) — The following can be set by calling `window.getProperties().put("attributeName", attributeValue)` in your JavaFX code:
    - **JavaFX Node Attributes** (17 lines 539-555) — These attributes can be set by calling `node.getProperties().put("attributeName", attributeValue)` in your JavaFX code.
  ...
  - Debug utilities (90 lines 602-691) — JPro servers provide a variety of utilities that can be useful for debugging or for system administrators to monitor ...
    - Restricting access (11 lines 668-678) — Access can be restricted by a `username` and `password`. Your
    - Additional utilities (13 lines 679-691) — > We find [ScenicView](https://github.com/JonathanGiles/scenic-view) very useful while developing, and it works perfe...
  - Logging (54 lines 692-745) — JPro provides the following a set of structured log messages out of the box:
    - Custom logback configurations (17 lines 705-721) — To override the default JPro logging configuration, simply provide a new `logback.xml` file and specify its path in t...
    - Logging to JSON (24 lines 722-745) — Enabling the `jpro.logToJsonFormat` option in your `jpro.conf` file will make JPro output its structured log messages...
  - Tips & Limitations (40 lines 746-785)
    - Potential issues (6 lines 748-753) — - Wait-, sleep-, and showAndWait commands in the JavaFX thread stop your app from being accessible. Dialogue boxes ar...
    - Working with stages (6 lines 754-759) — JavaFX **Stages** can be opened with the [WebAPI](/api/2026.1.1/jpro.webapi/com/jpro/webapi/WebAPI.html)’s `openStage...
    - Performance tips (10 lines 760-769) — Generally, the better your JavaFX app’s performance, the better its performance will be when running on the server & ...
    - Features with limited support (8 lines 770-777) — > JavaFX 3D support is currently in open beta.
    - Unsupported JavaFX features (8 lines 778-785) — - SwingNode
- Web Integration (170 lines 786-955)
  - Custom index.html (56 lines 788-843) — To make your JPro application accessible via URL, an “index.html” needs to exist in the project structure under `src/...
    ...
    - Specifying the app name in jpro.conf (13 lines 831-843) — In order to start the app specified in your `index.html`, the name specified in the `<jpro-app>` tag must be defined ...
  - Embedding JPro (59 lines 844-902) — JPro can be **embedded into an existing html-page** by using the  `<jpro-app>` tag, similar to how it is included in ...
    - Adding the JPro app tag to your HTML (21 lines 850-870) — First, the `jpro.js` script and `jpro.css` styling must be loaded from the server that is serving your JPro applicati...
...
- Configure external servers (149 lines 1190-1338) — one.jpro.loadbalancer.externalServer1=http://server1.example.com:9101
  ...
  - Configuration (80 lines 1220-1299)
    - An application.properties example (19 lines 1222-1240) — The file `application.properties` might look like the following:
    - Available Properties (25 lines 1241-1265) — The following parameters can be set in the `application.properties` file:
    - Configure `one.jpro.loadbalancer.userHome` property (23 lines 1266-1288) — The `one.jpro.loadbalancer.userHome` property controls how the user’s home directory is managed across JPro server in...
    - Logging properties (11 lines 1289-1299) — It is also possible to configure certain logging properties in the `application.properties`. For example:
...
- QF-Test Integration (72 lines 1339-1410)
  - The JPro Integration Library - jpro-utils.qft (71 lines 1340-1410) — The **jpro-utils.qft** library enables QF-Test to run automated tests directly
    - API Documentation (9 lines 1352-1360) — Full API documentation:
    - High-Level Overview (16 lines 1361-1376)
      - 1. `jpro.server` – Starting the JPro Server (6 lines 1363-1368) — QF-Test launches the JPro server (your JavaFX backend) and waits until it is ready.
      - 2. `jpro.client` – Launching the Browser (8 lines 1369-1376) — QF-Test starts a browser and loads your JPro application URL.
    - Quickstart Guide (29 lines 1377-1405)
      - 1. Include the Library (3 lines 1379-1381) — Add `jpro-utils.qft` as an *Included file* (just like `qfs.qft`).
      - 2. (Optional) Add Conditional Execution (3 lines 1382-1384) — If your suite also tests non-JPro apps, wrap the JPro setup in a condition.
      - 3. Add the `jpro.running` Dependency (21 lines 1385-1405) — Attach the dependency to your Testcase or TestcaseSet and configure:
    - Writing Tests (5 lines 1406-1410) — Once the JPro server and browser client are running, write your tests as usual:
