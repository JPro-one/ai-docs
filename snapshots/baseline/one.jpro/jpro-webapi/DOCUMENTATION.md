...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <properties>
        <jpro.version>2025.3.1-SNAPSHOT</jpro.version>
...
JPro currently supports Gradle versions 8.x and 9.x. Kotlin DSL is also supported.
...
Currently, JPro Maven Plugin has been tested with versions `3.6.3`, `3.8.8`, `3.9.9` and `4.0.0-rc-1`.
...
jpro {
  ...
  jproVersion = "2025.3.1-SNAPSHOT"  // The JPro version to use.  
...
For exact details about the API itself, please go to the [WebAPI-documentation](/api/2025.3.1-SNAPSHOT/jpro.webapi/com/jpro/webapi/WebAPI.html).
...
dependencies {
    ...
    implementation "one.jpro:jpro-webapi:2025.3.1-SNAPSHOT"
    ...
}
```
...
Here is the [download link](https://sandec.jfrog.io/artifactory/repo/com/sandec/jpro/jpro-webapi/2025.3.1-SNAPSHOT/jpro-webapi-2025.3.1-SNAPSHOT.jar) for the latest version.


## Custom HTTP Handlers

JPro allows you to attach custom request handlers to your website. To do so you have to add a request handler to the server API:
...
You can register this handler anywhere inside the code of your website, but a convinient way would be to register it on startup. This can be done by defining the *jpro.onFXStartup* flag inside your build.gradle file like this:

```
jpro.onFXStartup = one.jpro.hellojpro.StartServer
```

The corrosponding class should look something like this:
...
Custom HTTP handlers can be combined with the JPro load balancer to direct specific requests to specific JVMs. To do this, you first need to retrieve the instance ID:

```
WebAPI.getWebAPI(stage).getInstanceID(); // e.g. 82489-1-1-1
```

When you have the instance ID, you can add it as a query parameter to your request URL. The load balancer will then forward the request to the correct JVM:

```
http://localhost:8080/test?instanceID=82489-1-1-1
```
...
3. Download
   the [JPro Loadbalancer](https://sandec.jfrog.io/artifactory/repo/one/jpro/jpro-loadbalancer/2025.3.1-SNAPSHOT/jpro-loadbalancer-2025.3.1-SNAPSHOT.jar)
   into the folder F:
    ```shell
    curl -LO https://sandec.jfrog.io/artifactory/repo/one/jpro/jpro-loadbalancer/2025.3.1-SNAPSHOT/jpro-loadbalancer-2025.3.1-SNAPSHOT.jar
    ```
...
5. Put the zip file, created by the [JPro Release command](https://www.jpro.one/docs/current/2.1/JPRO_COMMANDS), into
   the folder F.
...
The `one.jpro.loadbalancer.userHome` property controls how the user’s home directory is managed across JPro server
instances. Below are the possible values and their descriptions:
...
* You can open **Stages** with the method `openStageAsPopup(Stage stage)` and `openStageAsTab(Stage stage)` of the
  [WebAPI](/api/2025.3.1-SNAPSHOT/jpro.webapi/com/jpro/webapi/WebAPI.html).
...
(this list refers to version 2025.3.1-SNAPSHOT)
...
* WebView (Supported in its basic form, only. We recommend to use the
  [HTMLView](/api/2025.3.1-SNAPSHOT/jpro.webapi/com/jpro/webapi/HTMLView.html), instead.)
...
## [Issue Tracker](https://github.com/JPro-one/JPro-Tickets)
