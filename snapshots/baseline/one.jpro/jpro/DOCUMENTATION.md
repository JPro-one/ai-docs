...
buildscript {
  ...
  dependencies {
    classpath "one.jpro:jpro-gradle-plugin:2026.1.1"
  }
}
```
...
| --- | --- | --- |
| jproVersion | The version of the `jpro-gradle-plugin` | The JPro-version to be used, for example: `2026.1.1` |
...
| jpro.liveCheck                            | "" | When calling `/status/alive` on the server, the main method of the defined class will be called. If it throws an exception, the server is considered not alive. |
| jpro.debug.delayWS                        | 0 | Adds an artificial delay (in milliseconds) before sending WebSocket messages to the browser. Useful for simulating slow network conditions during development. `0` means no delay. |
...
JavaFX **Stages** can be opened with the [WebAPI](/api/2026.1.1/jpro.webapi/com/jpro/webapi/WebAPI.html)’s `openStageAsPopup(Stage stage)` and `openStageAsTab(Stage stage)` methods. An alternative is to create new windows or dialogues using a StackPane at the root of your application.
...
For details about the API itself, please reference the [WebAPI-documentation](/api/2026.1.1/jpro.webapi/com/jpro/webapi/WebAPI.html).
...
dependencies {
    ...
    implementation "one.jpro:jpro-webapi:2026.1.1"
    ...
}
```
...
1. Create a new folder F and download the [JPro Loadbalancer](https://sandec.jfrog.io/artifactory/repo/one/jpro/jpro-loadbalancer/2026.1.1/jpro-loadbalancer-2026.1.1.jar) into it:
    ```shell
    curl -LO https://sandec.jfrog.io/artifactory/repo/one/jpro/jpro-loadbalancer/2026.1.1/jpro-loadbalancer-2026.1.1.jar
    ```
